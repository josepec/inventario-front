            ...
            <div class="shrink-0 p-4 border-t border-[#1a1a1a]">
              ...
              <div class="flex gap-2 mb-2">
                ...
                @if (detailTrackingMode() !== 1) {
                  <button
                    (click)="detailIsWanted() ? removeWanted(detail()!.id) : markWantedFromDetail()"
                    [disabled]="busyId() === detail()!.id"
                    class="group/wbtn relative flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-semibold disabled:opacity-50 transition-all duration-200"
                    [class]="detailIsWanted()
                      ? 'bg-[#7c3aed] text-white shadow-[0_0_12px_rgba(124,58,237,0.4)]'
                      : 'bg-transparent border border-[#2a2a2a] text-[#606060] hover:border-[#7c3aed]/50 hover:text-[#a78bfa]'">
                    @if (detailIsWanted()) {
                      <!-- Estado activo: relleno sólido, muestra "Quitar" en hover -->
                      <span class="flex items-center gap-1.5 group-hover/wbtn:hidden">
                      ...
                      <span class="hidden items-center gap-1.5 group-hover/wbtn:flex text-white/80">
                      ...
                      <!-- Estado inactivo: ghost, sin énfasis -->
...
export class NovedadesComponent implements OnInit {
  ...
  mineMain = computed(() => this.mine().filter(i =>
    ...
    const aw = (a.source === 'wanted' || a.wanted) ? 0 : 1;
    const bw = (b.source === 'wanted' || b.wanted) ? 0 : 1;
    return aw - bw;
  }));
